      ...
      {activeCategory === "All" && featured && (
        <section className="py-0 bg-background">
          <div className="container px-4 md:px-8 py-10 md:py-14">
            <motion.div {...fade} transition={{ duration: 0.5 }} className="rounded-2xl overflow-hidden grid md:grid-cols-5" style={{ backgroundColor: "#1A2B22" }}>
              <div className="md:col-span-3 p-8 md:p-10 flex flex-col justify-end text-white">
                <span className="text-xs uppercase tracking-[0.2em] font-medium mb-3" style={{ color: "#C4785A" }}>{featured.category}</span>
                ...
                <Link to={`/blog/${featured.slug}`} className="text-sm font-medium hover:underline underline-offset-4" style={{ color: "#C4785A" }}>Read More →</Link>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {gridPosts.map((post, i) => (
                <motion.article
                  ...
                  style={{ borderTop: "3px solid #C4785A" }}
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Link to={`/blog/${post.slug}`} className="text-sm font-medium hover:underline underline-offset-4" style={{ color: "#C4785A" }}>Read More →</Link>
                    </div>
                  </div>
                </motion.article>
              ))}
